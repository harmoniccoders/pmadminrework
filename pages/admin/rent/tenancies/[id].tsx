...
import TenancyAgreement from "lib/components/Utilities/TenancyAgreement";
  ...
  return (
    <Box bgColor="white" px="2rem" minH="80vh" pb="2rem" pos="relative">
      ...
      <ViewTenantComplaints
        ...
        data={singleTenancy}
      />
      <TenancyAgreement data={tenancy} isOpen={open} onClose={close} />
    </Box>
  );
}
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  try {
    const tenancy = (await _dataAccess.get(`/api/tenancy/${id}`)).data;
    console.log({ tenancy });
